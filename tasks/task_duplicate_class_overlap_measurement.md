...
> (로컬 경로: `data/CICIDS2017_CNS2022_reprocessed/` — 요일별 CSV 5개)
> **작성일:** 2026-09-13 / **개정:** 2026-09-13 (§3 `Protocol` 오기 정정)
> **관련 결정:** D-001(Attempted 유보), D-002(DoS Hulk 관찰군), D-003(피처 구성 59/32)
> **관련 미결 항목:** `docs/dataset_audit_2026-09-11.md` §5 "class overlap이 우리 데이터에서 몇 건인지 측정"
...
**`configs/features_whitelist.json`의 `include` 59개 컬럼만** 비교 키로 사용한다.
목록을 이 문서에 옮겨 적지 않는다. 코드가 그 파일을 읽어 키를 구성한다.

### 3-1. 비교 키에서 빠지는 컬럼 — 확인용

아래는 `exclude` 32개 중 **혼동하기 쉬운 항목**이다. 비교 키에 들어가면 안 된다.
모델이 보지 않는 컬럼으로 중복을 판정하면 이 측정의 의미가 없어진다.

`Flow ID`, `Src IP`, `Dst IP`, `Src Port`, `Dst Port`, `Timestamp`, `id`,
`Label`, `Attempted Category`

### 3-2. `Protocol`은 include다 — 비교 키에 **포함**한다

> **정정 기록 (2026-09-13).** 이 문서의 초판은 `Protocol`을 위 제외 목록에 넣었다. **오기였다.**
> `Protocol`은 `configs/features_whitelist.json`의 `include` 59개에 속한다
> (D-003 §6). 초판은 같은 절에서 "include 59개만 사용"과 "Protocol 제외"를 동시에 지시해
> 자기모순이었고, 그대로 실행하면 58개 컬럼으로 비교해 **측정 결과가 무효가 된다.**
> 경위: `docs/doc_consistency_audit_2026-09-13.md` §A-1.

`Protocol`을 비교할 때 **값을 정수가 아니라 범주로 다룬다.**
D-003 §6의 매핑을 적용한 뒤 비교한다.
...
| 17 | `UDP` |
| 1 | `ICMP` |
| 0 | `UNKNOWN` (TSO 아티팩트 표식) |

중복 판정에서는 정수 비교와 범주 비교의 결과가 같지만(값이 같으면 범주도 같다),
후속 코드가 이 키 구성을 재사용할 수 있으므로 처음부터 범주로 다룬다.
...
- `Flow Bytes/s`, `Flow Packets/s`에 `+Infinity` 10건이 있다. 별도로 취급하지 말고
  ...
  (`Flow Packets/s`는 whitelist `exclude`이므로 실제 키에는 `Flow Bytes/s`의 5건만 들어간다.
  이 사실도 보고서에 적는다.)
...
7. 충돌 그룹 중 크기 상위 20개의 표본 — 각 그룹의 행 식별 정보(요일 + `id`), `Label`,
   그리고 59개 값 중 주요 몇 개
...
3. 피처 목록을 코드에 하드코딩하지 말 것. `configs/features_whitelist.json`을 읽는다.
...
6. `_superseded/` 아래 코드를 재사용하지 말 것. 새로 작성한다.
