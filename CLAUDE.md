...
- 이 하위실험의 질문은 하나다 — **"1차 게이트에 유보 구간이 실제로 존재하는가."**
  성능 측정이 아니라 존재 주장이다.
- 판정 지표는 **AURC**다. AURC는 확정된 confidence의 순위와 동률을 보존하는 엄격한 단조변환에
  불변이다. 이 실험은 confidence 순위 평가에 한정하므로 확률 보정(calibration)을 수행하지 않는다.
  **ECE/Brier는 범위 밖이다.**
- 유보 구간의 폭은 미정이다. test 예측 확률을 전량 저장해 **오프라인으로 스윕**한다.
  사전 임계값을 정하지 않는다.
- 게이트를 신뢰도 단독으로 구성할지 OOD 보조 게이트를 병행할지는 미정이다.
- 2·3차 계층(LLM 검증, RAG, Critic)은 아직 구현 대상이 아니다.
  요청받지 않은 한 해당 코드를 작성하지 않는다.

59-feature 중복·class overlap 측정과 D-004 split 고정은 완료됐다.
지금 해야 할 일은 D-006 학습·평가 프로토콜과 성공/실패/판정불가 기준 확정이다.
`docs/CURRENT_DECISIONS.md`의 "다음 작업" 절을 먼저 읽는다.
