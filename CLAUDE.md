...
- 이 하위실험의 질문은 하나다 — **"1차 게이트에 유보 구간이 실제로 존재하는가."**
  성능 측정이 아니라 존재 주장이다.
- 판정 지표는 **AURC**다. AURC는 단조변환 불변이므로 보정 전에 계산된다.
  **따라서 이 실험은 확률 보정(calibration)을 수행하지 않는다. ECE/Brier는 범위 밖이다.**
...
지금 해야 할 일은 `tasks/task_duplicate_class_overlap_measurement.md`다.
